...
void workloadOperation(rocksdb::DB* db, string const &operation, Stats &stats,
                       ...
                       rocksdb::Status &status, vector<string> &get_buffer,
    ...
    if (input.is_open()) {
      ...
      while (getline(input, line)) {
        ...
        workloadOperation(db, operation, stats, key, value, status,
                          get_buffer,deletes, omit_deletes);
...
//    assert(status.ok());
  ...
  ofstream outfile;
  if(omit_deletes) {
    outfile.open("raw_data/gadget_1M_workload_buffer" + to_string(buffer_threshold) + "_no_deletes.txt");
  }
  else{
    outfile.open("raw_data/gadget_1M_workload_buffer" + to_string(buffer_threshold) + "_with_deletes.txt");
  }
  if(outfile.is_open()) {
